...
static uint32_t emios_overflow_count[EMIOS_ICU_IP_INSTANCE_COUNT][EMIOS_ICU_IP_NUM_OF_CHANNELS];
static uint8_t emios_inst, emios_hw_ch_cw, emios_hw_ch_ccw;
...
static int qdec_s32_fetch(const struct device *dev, enum sensor_channel ch)
{
	...
	data->abs_counter = (int32_t)(
		+(data->counter_CW  + EMIOS_ICU_IP_COUNTER_MASK *
			emios_overflow_count[emios_inst][emios_hw_ch_cw])
		-(data->counter_CCW + EMIOS_ICU_IP_COUNTER_MASK *
			emios_overflow_count[emios_inst][emios_hw_ch_ccw]));

	LOG_DBG("ABS_COUNT = %d CW = %u OverFlow_CW = %u CCW = %u Overflow_CCW = %u",
		data->abs_counter, data->counter_CW,
		emios_overflow_count[emios_inst][emios_hw_ch_cw],
		data->counter_CCW, emios_overflow_count[emios_inst][emios_hw_ch_ccw]);
