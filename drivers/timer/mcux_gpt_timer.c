 ...
 * Choosing a fraction of a tick is probably a good enough
...
void sys_clock_set_timeout(int32_t ticks, bool idle)
{
	...
	if ((next - now) < MIN_DELAY) {
		/* Move next up by a cycle, to be sure we don't miss announcement */
		next += CYC_PER_TICK;
	}
	/*
	 * Set a hard cap on the match value here. Since we always get an
	 * interrupt at the counter rollover, there is no risk
	 * of missing the time advancement of the counter if this
	 * new match value is less than MIN_DELAY
	 */
